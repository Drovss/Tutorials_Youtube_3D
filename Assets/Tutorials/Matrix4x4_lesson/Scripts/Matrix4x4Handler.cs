...
{
    public static class Matrix4x4Handler
    {
	    public static string ToStringMatrix4x4(this Matrix4x4 matrix, bool isRound = false)
	    {
		    ...
		    for (int i = 0; i < 4; i++)
		    {
			    ...
			    {
				    if (isRound)
					    text += Mathf.RoundToInt(matrix[i, j]).ToString();
				    else
					    text += matrix[i, j].ToString(" 0.0000000");
		...
		public static Matrix4x4 HouseholderReflection(this Matrix4x4 matrix4X4, Vector3 planeNormal)
		{
			...
			Matrix4x4 householderMatrix = Matrix4x4.identity.Minus(
				MultiplyVectorsTransposed(planeNormal4, planeNormal4).MultiplyByNumber(2));
